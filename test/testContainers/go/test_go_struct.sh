...
# of their fields.
STRUCT_FILE=readelf.dump
readelf --debug-dump=info net/tlsServer > $STRUCT_FILE
...
function check_expected_file_arg() {
  ...
  if [ ! -f "$STRUCT_PATH" ]; then
      echo "test_go_struct.sh cannot access $STRUCT_PATH"
      echo "Example contents of $STRUCT_PATH:"
      echo " runtime.g|m=48"
      echo " runtime.m|tls=96"
      echo " net/http.connReader|conn=0"
      echo " net/http.conn|tlsState=48"
...
function test_it() {
  ...
  FIELD_OFFSET=$(cat $STRUCT_PATH | grep "$STRUCT_NAME|$FIELD_NAME" | cut -d= -f2)

  if [ -z "$FIELD_OFFSET" ]; then
      echo "  Failed to find expected offset value '$STRUCT_NAME|$FIELD_NAME' in $STRUCT_PATH"
  ...
  STRUCT_LINE_NUMS=$(grep -A1 -n DW_TAG_structure_type $STRUCT_FILE | grep -B1 -A3 ": ${STRUCT_NAME}$")
  ...
  STRUCT_RAW_OUT=$(cat $STRUCT_FILE | sed -n "${FIRST_LINE_NUM},${LAST_LINE_NUM}p")
...
# This is a complete list of the offsets we're using in scope library for go
#
#       STRUCT_NAME                   FIELD_NAME
#       DW_TAG_structure_type         DW_TAG_member
test_it "runtime.g"                   "m"
test_it "runtime.m"                   "tls"
test_it "net/http.connReader"         "conn"
test_it "net/http.conn"               "tlsState"

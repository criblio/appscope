...
if [[ $raw != "" ]]; then
    ...
    $SCOPE_PATH/scope events -aj | grep '"open"' | jq -r '.data.file' | xargs dirname | sort | uniq  > $RESULT    
else
    $SCOPE_PATH/scope events -aj | grep '"open"' | jq -r '.data.file' | sort | uniq > $RESULT
fi

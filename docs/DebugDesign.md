# AppScope Crash Analysis

## Customer Use Case / Problem Description

When a process crashes it can be a challenge to get enough information to help developers troubleshoot the situation. Some examples of the difficulties... it can be hard to reproduce the crash outside the environment where the crash was first seen. Container environments are typically very transient so it can be tough to obtain shell access. Container storage that might give clues may not persist after the container has exited. Installation of debugging tools may be challenging or undesirable. Even outside of containers the procedure to turn on kernel-provided coredumps often varies by linux distro and can be a security liability (particularly with DOS attacks). It can be challenging to know whether AppScope is playing a role in the crash itself.

We expect that the improvements discussed here will help AppScope developers. The same improvements can be used by users who may not use AppScope today, but who have observed crashes in their environments and need help to capture debugging information.
...
### Take a snapshot (coredump, backtrace, etc)
...
Ideally our implementation would allow the AppScope library and AppScope cli to operate completely independently so that users are not required to run both. For our MVP implementation we're not planning for this to be 100% true. For MVP we don't plan to allow the AppScope library to run without support from the AppScope cli. This leaves three debuggging modes of operation.

0. Without AppScope involved in any way, users may configure the kernel to take crash traces. This will be a mode of operation that people will be familiar with.

1. The AppScope cli runs in a daemonized mode and the AppScope library is not actively loaded in any processes. In this state the cli can produce data based from the kernel (eBPF signals), and can capture much of the data described in the [Debug Information](#ok-but-what-is-in-this-snapshot-thing) section above. AppScope will not make stacktraces nor coredumps available in this mode. After MVP, we could consider having the AppScope cli retrieve coredumps that may be written by the kernel. After MVP, we may allow the cli to capture information on command from a running process (without requiring that some process actually be crashing).

2. The AppScope cli runs in a daemonized mode and the AppScope library is actively loaded in processes. This interaction is described in more detail in the next section.

### Some simplifying assumptions for an MVP Implementation

Whenever containers are involved, the cli is required to be able to retrieve and persist the snapshot. And, as was discussed above, the library must be involved to stop the world so that coredumps or backtraces can be retrieved before containers are terminated.

Assuming that we can accept that both the cli and library will be required and that we want to make the most use of work done during technical feasibility, the MVP solution may look like this. This assumes the snapshot feature has been enabled.
  - User runs a cli command to start a daemon on the host that uses eBPF to detect signals
  - a scoped process is configured with a snapshot feature enabled
...
These simplifying assumptions for MVP require that:
  ...
  - we allow the cli to run in a daemonized mode
  - it is allowable for the library to stop it's process indefinitely if this feature is enabled and a cli daemon is not running (not one of the three supported modes of operation above.)

### Security concerns

We need to consider the Debug Information listed above from a security perspective. Coredumps will almost certainly have the potential to leak information. Capture of backtraces, environment variables, and process arguments should be considered in a similar light. We plan to have the snapshot capability built into AppScope. This said, we will certainly provide a way to control if the feature will be enabled. We plan to disable the snapshot feature by default. User action will be required to explicitly enable the feature.

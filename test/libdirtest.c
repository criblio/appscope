...
CreateDirIfMissingWrongPathNull(void **state) {
    mkdir_status_t res = libdirCreateDirIfMissing(NULL, 0777, scope_getuid(), scope_getgid());
    assert_int_equal(res, MKDIR_STATUS_ERR_NOT_ABS_DIR);
}

static void
CreateDirIfMissingWrongPathCurrentDir(void **state) {
    mkdir_status_t res = libdirCreateDirIfMissing(".", 0777, scope_getuid(), scope_getgid());
    assert_int_equal(res, MKDIR_STATUS_ERR_NOT_ABS_DIR);
}
...
CreateDirIfMissingWrongPathFile(void **state) {
    ...
    mkdir_status_t status = libdirCreateDirIfMissing(buf, 0777, scope_getuid(), scope_getgid());
...
CreateDirIfMissingPermissionIssue(void **state) {
    mkdir_status_t res = libdirCreateDirIfMissing("/root/loremIpsumFile/", 0777, scope_getuid(), scope_getgid());
...
CreateDirIfMissingAlreadyExists(void **state) {
    ...
    assert_int_equal(res, 0);
    mkdir_status_t status = libdirCreateDirIfMissing(dirName, 0777, scope_getuid(), scope_getgid());
    assert_int_equal(status, MKDIR_STATUS_ERR_NOT_ABS_DIR);
    status = libdirCreateDirIfMissing(buf, 0777, scope_getuid(), scope_getgid());
...
CreateDirIfMissingAlreadyExistsPermIssue(void **state) {
    mkdir_status_t status = libdirCreateDirIfMissing("/root", 0777, scope_getuid(), scope_getgid());
...
CreateDirIfMissingSuccessCreated(void **state) {
    ...
    res = rm_recursive(buf);
    assert_int_equal(res, 0);
     status = libdirCreateDirIfMissing(buf, 0755, scope_getuid(), scope_getgid());
    assert_int_equal(status, MKDIR_STATUS_CREATED);
    res = scope_stat(buf, &dirStat);
    assert_int_equal(res, 0);
    assert_int_equal(S_ISDIR(dirStat.st_mode), 1);
...
SetLibrarySuccessDev(void **state) {
    libdirInitTest(TEST_INSTALL_BASE, TEST_TMP_BASE, "dev");
    // Create dummy file
    mkdir_status_t mkres = libdirCreateDirIfMissing("/tmp/appscope-test/success/dev", 0777, scope_getuid(), scope_getgid());
...
ExtractNewFileDev(void **state) {
    ...
    int res = libdirExtract(LIBRARY_FILE, scope_getuid(), scope_getgid());
    assert_int_equal(res, 0);
    ...
    res = libdirExtract(LOADER_FILE, scope_getuid(), scope_getgid());
    assert_int_equal(res, 0);
...
ExtractFileExistsOfficial(void **state) {
    ...
    res = libdirExtract(LIBRARY_FILE, scope_getuid(), scope_getgid());
    assert_int_equal(res, 0);
    scope_snprintf(expected_location, PATH_MAX, "%s/%s/%s", TEST_INSTALL_BASE, normVer, "libscope.so");
...
GetPathDev(void **state) {
    ...
    mkdir_status_t mkres = libdirCreateDirIfMissing(expected_location, 0777, scope_getuid(), scope_getgid());
    assert_in_range(mkres, MKDIR_STATUS_CREATED, MKDIR_STATUS_EXISTS);
    // Create dummy file
    scope_memset(expected_location, 0, PATH_MAX);
    scope_snprintf(expected_location, PATH_MAX, "%s/%s/%s", TEST_TMP_BASE, normVer, "libscope.so");

...
void
javaAddMethod(java_class_t *info, const char *name, const char *descriptor, uint16_t accessFlags, uint16_t maxStack, uint16_t maxLocals, uint8_t *code, uint32_t codeLen)
{
    ...
    */
    ...
    // write method info
    *((uint16_t *)addr) = htobe16(accessFlags);
    addr += 2;
    *((uint16_t *)addr) = htobe16(nameIndex);
    addr += 2;
    *((uint16_t *)addr) = htobe16(descriptorIndex);
    addr += 2;
    *((uint16_t *)addr) = htobe16(attrCount);
    addr += 2;
    ...
    if (code != NULL) {
        ...
        *((uint16_t *)addr) = htobe16(codeNameIndex);
        addr += 2;
        *((uint32_t *)addr) = htobe32(codeAttrLen);
        addr += 4;
        *((uint16_t *)addr) = htobe16(maxStack);
        addr += 2;
        *((uint16_t *)addr) = htobe16(maxLocals);
        addr += 2;
        *((uint32_t *)addr) = htobe32(codeLen);
        addr += 4;
        memcpy(addr, code, codeLen);
        addr += codeLen;
        *((uint16_t *)addr) = htobe16(exceptionLen);
        addr += 2;
...
javaAddField(java_class_t *info, const char *name, const char *descriptor, uint16_t accessFlags)
{
    ...
    *((uint16_t *)buf) = htobe16(accessFlags);
    buf += 2;
    *((uint16_t *)buf) = htobe16(nameIndex);
    buf += 2;
    *((uint16_t *)buf) = htobe16(descriptorIndex);
    buf += 2;
...
javaWriteClass(unsigned char *dest, java_class_t *info)
{
    ...
    addr += sizeof(magic);
    *((uint16_t *)addr) = htobe16(info->minor_version);
    addr += 2;
    *((uint16_t *)addr) = htobe16(info->major_version);
    addr += 2;
    *((uint16_t *)addr) = htobe16(info->constant_pool_count);
    addr += 2;
    int i;
    ...
    *((uint16_t *)addr) = htobe16(info->access_flags);
    addr += 2;
    *((uint16_t *)addr) = htobe16(info->this_class);
    addr += 2;
    *((uint16_t *)addr) = htobe16(info->super_class);
    addr += 2;
    *((uint16_t *)addr) = htobe16(info->interfaces_count);
    addr += 2;
    memcpy(addr, info->interfaces, info->interfaces_count * 2);
    addr += info->interfaces_count * 2;
    // write fields
    *((uint16_t *)addr) = htobe16(info->fields_count);
    addr += 2;
    ...
    *((uint16_t *)addr) = htobe16(info->methods_count);
    addr += 2;
    // write methods
    for (i = 0; i < info->methods_count; i++) {
    ...
    // write attributes
    *((uint16_t *)addr) = htobe16(info->attributes_count);
    addr += 2;
...
javaReadClass(const unsigned char *classData)
{
    ...
    classInfo->minor_version = be16toh(*((uint16_t *)off));
    off += 2;
    classInfo->major_version = be16toh(*((uint16_t *)off));
    off += 2;
    classInfo->constant_pool_count = be16toh(*((uint16_t *)off));
    off += 2;
    ...
    classInfo->access_flags = be16toh(*((uint16_t *)off));
    off += 2;
    classInfo->this_class = be16toh(*((uint16_t *)off));
    off += 2;
    classInfo->super_class = be16toh(*((uint16_t *)off));
    off += 2;
    classInfo->interfaces_count = be16toh(*((uint16_t *)off));
    off += 2;
    classInfo->interfaces = (uint16_t *)off;
    off += classInfo->interfaces_count * 2;

    // read fields
    classInfo->fields_count = be16toh(*((uint16_t *)off));
    off += 2;
    ...
    // read methods
    classInfo->methods_count = be16toh(*((uint16_t *)off));
    off += 2;
    ...
    // read attributes
    classInfo->attributes_count = be16toh(*((uint16_t *)off));
    off += 2;
...
void
javaDestroy(java_class_t **classInfo)
{
    int i;

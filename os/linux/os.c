...
"    The Scope library is capable of being loaded into any process as the process starts.\n"
"    The primary means of defining which processes include the Scope library is by exporting the\n"
"    the environment variable LD_PRELOAD. The LD_PRELOAD variable is set to point to the path\n"
"    name of the Scope library.\n"
"\n"
"    Scope emits data as metrics and/or events.\n"
"    Scope is fully configurable by means of a configuration file and/or environment variables.\n"
"\n"
"    Metrics are emitted in StatsD format over a configurable link. By default metrics are sent over\n"
"    a UDP socket using localhost and port 8125.\n"
"\n"
"    Events are emitted in JSON format over a configurable link. By default events are sent over\n"
"    a TCP socket using localhost and port 9109.\n"
"\n"
"    Scope logs to a configurable destination along with a configurable verbosity level. The default\n"
"    verbosity setting is level 4 and the default destination is the file /tmp/scope.log.\n"
"\n"
"CONFIGURATION:\n"
"    Configuration File:\n"
...
"        first one it finds.  If this fails too, it will look for scope.yml in the\n"
...
"     In contrast to metrics, event content is not managed with verbosity settings.\n"
"     Event content is managed by means of regex filters that manage which specific metric\n"
"     is to be included, what field types should be included and what values in fields should\n"
"     be included.\n"
"\n"
"     Events are organized as 4 watch types: \n"
"     1) File Content. Provide a pathname and all data written to the file will be organized\n"
"        in JSON format and emitted over the event channel.\n"
"     2) Console Output. Select stdin &/or stdout and all data written to these endpoints will\n"
"        be formatted in JSON and emitted over the event channel\n"
"     3) Metrics. Content that is much the same as that extracted for StatsD metrics is formatted\n"
"        in JSON and emitted over the event channel. Metrics emitted as events are not summarized\n"
"        and controlled with verbosity settings. Rather regex filters manage which\n"
"        metric, speficic fields in a metric and value patterns within a field.\n"
"     4) HTTP Headers. HTTP headers are extracted, formatted in JSON and emitted over the event\n"
"        channel. There are 3 types of events created for HTTP headers; 1) HTTP request events,\n"
"        HTTP response events and a metric event corresponding to the request and response\n"
"        sequence. A response event includes the corresponding request, status and duration fields.\n"
"        An HTTP metric event provides fields describing bytes received, requests per second,\n"
"        duration and status.\n"
"\n"
"PROTOCOL DETECTION:\n"
"     Any definbed network protocol can be detected by Scope. A protocol definition is provided in a\n"
"     separate YAML confg file. Protocol specifics are described in one or more regex definitions.\n"
"     PCRE2 regular expressions are supported. Binary and string protocols are detected. Detection\n"
"     events formatted in JSON are emitted over the event channel.\n"
"\n"
"     The event metric watch type should be enabled in order for protocols to be detected.\n"
"\n"
"     The protocol detection config file should be named scope_protocol.yml. The file should be placed\n"
"     in the directory defined by the SCOPE_HOME environment variable. Alternatively, the protocol\n"
"     definition config file can be found in the same search order as that described for config files.\n"
"\n";

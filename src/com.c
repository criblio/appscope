...
sendProcessStartMetric()
{
    ...
    event_field_t fields[] = {STRFIELD("proc", (g_proc.procname), 4, TRUE), NUMFIELD("pid", (g_proc.pid), 4, TRUE), STRFIELD("host", (g_proc.hostname), 4, TRUE),
                              STRFIELD("args", (command), 7, TRUE),         STRFIELD("unit", ("process"), 1, TRUE), FIELDEND};
...
reportProcessStart(ctl_t *ctl, bool init, which_transport_t who)
{
    ...
    if (g_sendprocessstart && ((who == CFG_LS) || (who == CFG_WHICH_MAX)) && cfgLogStream(g_cfg.staticfg)) {
...
{
    if (!ctlEvtSourceEnabled(ctl, CFG_SRC_HTTP))
        return 0;
...
{
    cJSON *json;
    if (!str || !(json = cJSON_CreateString(str)))
        return -1;
...
jsonProcessObject(proc_id_t *proc)
{
    ...
    if (!(cJSON_AddStringToObjLN(root, "libscopever", SCOPE_VER)))
        goto err;

    if (!(cJSON_AddNumberToObjLN(root, "pid", proc->pid)))
        goto err;
    if (!(cJSON_AddNumberToObjLN(root, "ppid", proc->ppid)))
        goto err;
    if (!(cJSON_AddStringToObjLN(root, "hostname", proc->hostname)))
        goto err;
    if (!(cJSON_AddStringToObjLN(root, "procname", proc->procname)))
        goto err;
    if (proc->cmd) {
        if (!(cJSON_AddStringToObjLN(root, "cmd", proc->cmd)))
            goto err;
    }
    if (!(cJSON_AddStringToObjLN(root, "id", proc->id)))
        goto err;
    // starttime
...
msgLogConfig(config_t *cfg)
{
    ...
    if (!(json = jsonConfigurationObject(cfg)))
        return;
...
msgStart(proc_id_t *proc, config_t *cfg, which_transport_t who)
{
    ...
    if (cfgAuthToken(cfg)) {
        if (!cJSON_AddStringToObjLN(json_root, "authToken", cfgAuthToken(cfg)))
            goto err;
    }

    if (who == CFG_LS) {
        if (!cJSON_AddStringToObjLN(json_root, "format", "scope"))
            goto err;
    } else {
        if (!cJSON_AddStringToObjLN(json_root, "format", "ndjson"))
            goto err;
        if (checkEnv("SCOPE_CRIBL_NO_BREAKER", "true")) {
            if (!cJSON_AddStringToObjLN(json_root, "breaker", "Cribl - Do Not Break Ruleset"))
                goto err;
        }
    }

    if (!(json_info = cJSON_AddObjectToObjLN(json_root, "info")))
        goto err;

    if (!(json_proc = jsonProcessObject(proc)))
        goto err;
    cJSON_AddItemToObjectCS(json_info, "process", json_proc);

    if (!(json_cfg = jsonConfigurationObject(cfg)))
        goto err;
    cJSON_AddItemToObjectCS(json_info, "configuration", json_cfg);

    if (!(json_env = jsonEnvironmentObject()))
        goto err;
...
int
pcre2_match_wrapper(pcre2_code *re, PCRE2_SPTR data, PCRE2_SIZE size, PCRE2_SIZE startoffset, uint32_t options, pcre2_match_data *match_data, pcre2_match_context *mcontext)
{
...
int
regexec_wrapper(const regex_t *preg, const char *string, size_t nmatch, regmatch_t *pmatch, int eflags)
{
    if (g_need_stack_expand == FALSE) {

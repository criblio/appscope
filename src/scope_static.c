...
get_dir(const char *path, char *fres, size_t len)
{
    ...
    while ((entry = readdir(dirp)) != NULL) {
        if ((entry->d_type != DT_DIR) && (strstr(entry->d_name, fname))) {
...
set_library(const char *libpath)
{
    ...
    buf = mmap(NULL, ROUND_UP(sbuf.st_size, sysconf(_SC_PAGESIZE)), PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, (off_t)NULL);
    if (buf == MAP_FAILED) {
    ...
    if (elf->e_ident[EI_MAG0] != ELFMAG0 || elf->e_ident[EI_MAG1] != ELFMAG1 || elf->e_ident[EI_MAG2] != ELFMAG2 || elf->e_ident[EI_MAG3] != ELFMAG3 || elf->e_ident[EI_VERSION] != EV_CURRENT) {
...
set_loader(char *exe)
{
    ...
    for (i = 0; i < elf->e_phnum; i++) {
        if ((phead[i].p_type == PT_INTERP)) {
            ...
            while ((entry = readdir(dirp)) != NULL) {
                if ((entry->d_type != DT_DIR) && (strstr(entry->d_name, "ld-musl"))) {
...
get_loader(char *exe)
{
    ...
    buf = mmap(NULL, ROUND_UP(sbuf.st_size, sysconf(_SC_PAGESIZE)), PROT_READ, MAP_PRIVATE, fd, (off_t)NULL);
    if (buf == MAP_FAILED) {
...
do_musl(char *exld, char *ldscope)
{
    ...
    if ((symlink((const char *)exld, lpath) == -1) && (errno != EEXIST)) {
...
setup_loader(char *ldscope)
{
    ...
    if (((ldso = get_loader(EXE_TEST_FILE)) != NULL) && (strstr(ldso, LIBMUSL) != NULL)) {
...
static const char scope_help_overview[] = "  OVERVIEW:\n"
...
static const char scope_help_configuration[] = "  CONFIGURATION:\n"
                                               "    Configuration File:\n"
...
static const char scope_help_metrics[] = "  METRICS:\n"
...
static const char scope_help_events[] = "  EVENTS:\n"
...
static const char scope_help_protocol[] = "  PROTOCOL DETECTION:\n"
...
{
    printf("Cribl AppScope Static Loader %s\n"
           "\n"
...
static struct option opts[] = {{"usage", no_argument, 0, 'u'},        {"help", optional_argument, 0, 'h'},
                               {"attach", required_argument, 0, 'a'}, {"libbasedir", required_argument, 0, 'l'},
                               {"patch", required_argument, 0, 'p'},  {0, 0, 0, 0}};

...
{
    for (int i = 0; i < len; i++) {
        scopeLogInfo("%02x ", *(addr + i));
        if ((i + 1) % 4 == 0) scopeLogInfo("  ");
        if ((i + 1) % 16 == 0) scopeLogInfo("\n");
    }
    scopeLogInfo("\n");
}

void 
javaPrintClassInfo(java_class_t *info)
{
    scopeLogInfo("Version %d.%d\n", info->major_version, info->minor_version);
    scopeLogInfo("CONSTANT POOL, count = %d\n", info->constant_pool_count);
    scopeLogInfo("IDX\tTAG\t%-20s\tLEN\tCONTENTS\n", "NAME");
    for (int i = 1; i < info->constant_pool_count; i++) {
        ...
        scopeLogInfo("%3d\t%2d\t%-20s\t%d\t%s\n", i, tag, cp_tag_name[tag], len, buf);
    }
    scopeLogInfo("\nINTERFACES count=%d\n", info->interfaces_count);

    scopeLogInfo("\nFIELDS count=%d\n\n", info->fields_count);
    scopeLogInfo("FLAGS\tNAME IDX\tDESC IDX\tATTR COUNT\n");
    for (int i = 0; i < info->fields_count; i++) {
        ...
        for (int j = 0; j < attributes_count; j++) {
            ...
            scopeLogInfo("attribute_name_index=%d, attribute_length=%d\n", attribute_name_index, attribute_length);
        }
    }
        ...
        for (int j = 0; j < attributes_count; j++) {
            ...
            if (strcmp(attr_name, "Code") == 0)  {
                ...
                */
                ...
                scopeLogInfo("CODE name_index=%d, length=%d, max_stack=%d, max_locals=%d, code_length=%d, exception_table_length=%d\n",
                       attr_name_index, attr_length, max_stack, max_locals, code_length, exception_table_length);
                printCode(off + 8, code_length);
            } else {
                scopeLogInfo("attribute name=%s, name_index=%d, length=%d\n", attr_name, attr_name_index, attr_length);
            }
            off += attr_length;
            free(attr_name);
        }

...
> Note: LogStream's AppScope source can parse HTTP/1 and HTTP/2 payloads and
> generate corresponding events but the fields in those events differ from
> what's described here. The events generated by AppScope can be identified
> currently by their lack of timestamps. Payloads need to be enabled globally or
> for HTTP specifically in the protocol configs for that to happen.

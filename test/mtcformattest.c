...
{
    event_field_t fields[] = {STRFIELD("proc", "redis", 2, TRUE), FIELDEND};
...
mtcFormatEventForOutputHappyPathStatsd(void **state)
{
    ...
    event_field_t fields[] = {STRFIELD("proc", g_procname, 2, TRUE),
    ...
    char expected[1024];
    int rv = snprintf(expected, sizeof(expected), "net.port:%d|g|#proc:%s,pid:%d,fd:%d,host:%s,proto:%s,port:%d\n", g_openPorts, g_procname, pid, fd, g_hostname, proto, localPort);
...
mtcFormatEventForOutputJsonWithCustomFields(void **state)
{
    ...
    event_field_t fields[] = {STRFIELD("proc", g_procname, 2, TRUE), FIELDEND};
...
mtcFormatEventForOutputHappyPathFilteredFields(void **state)
{
    ...
    char expected[1024];
    int rv = snprintf(expected, sizeof(expected), "net.port:%d|g|#proc:%s,pid:%d,proto:%s,port:%d\n", g_openPorts, g_procname, pid, proto, localPort);
    assert_true(rv > 0 && rv < 1024);
    assert_string_equal(expected, msg);
    free(msg);
...
mtcFormatEventForOutputWithCustomAndStatsdFields(void **state)
{
    ...
    event_field_t fields[] = {STRFIELD("proc", "test", 2, TRUE), FIELDEND};
...
{
    event_field_t fields[] = {NUMFIELD("J", 222, 9, TRUE),
                              STRFIELD("I", "V", 8, TRUE),
...
{
    event_field_t fields[] = {STRFIELD("A", "Z", 0, TRUE),
                              NUMFIELD("B", 987, 1, TRUE),
                              STRFIELD("C", "Y", 2, TRUE),
                              NUMFIELD("D", 654, 3, TRUE),
                              STRFIELD("E", "X", 4, TRUE),
...
fmtUrlEncodeDecodeRoundTrip(void **state)
{
    ...
    test_case_t round_trip_tests[] = {
        ...
        {.decoded = "bar|pound#colon:ampersand@comma,end", .encoded = "bar%7Cpound%23colon%3Aampersand%40comma%2Cend"},
        // Other misc tests
        {.decoded = "", .encoded = ""},
        {.decoded = "a b", .encoded = "a%20b"},
        {.decoded = "행운을", .encoded = "%ED%96%89%EC%9A%B4%EC%9D%84"},
    };
...
fmtUrlDecodeToleratesBadData(void **state)
{
    ...
    test_case_t bad_input_tests[] = {
        ...
        {.decoded = "", .encoded = "%G0"},
        {.decoded = "", .encoded = "%0g"},
        {.decoded = "hey", .encoded = "hey%0gyou"},
        {.decoded = "hey", .encoded = "hey%%0you"},
        {.decoded = "hey", .encoded = "hey%1gyou"},
        {.decoded = "hey", .encoded = "hey%%1you"},
    };

...
send_multiple_rights(int sd, int *sendfds, int numfds)
{
    ...
    if ((control = malloc(CMSG_LEN(csize))) == NULL)
        return -1;
    ...
    *(int *)CMSG_DATA(cmptr) = sendfds[1];
    ...
    fprintf(stderr, "cmsghdr %ld control 0x%p ctl len %ld msg1 0x%p msg2 0x%p\n", sizeof(struct cmsghdr), msg.msg_control, msg.msg_controllen, CMSG_FIRSTHDR(&msg), CMSG_NXTHDR(&msg, cmptr));
...
main(int argc, char **argv)
{
    ...
    while (1) {
        ...
        // get the send fd
        if ((sendfd = get_send_fd()) == -1)
            exit(-1);

        // get a socket to send
        if ((sendsock = get_send_sock()) == -1)
            exit(-1);

        // send fd access rights
        switch (ttype) {
            case 1:
                if (send_access_rights(sd, sendfd) == -1)
                    exit(-1);
                break;
            case 2:
                if (send_access_rights(sd, sendsock) == -1)
                    exit(-1);
                break;
            case 3:
                sendfds[0] = sendfd;
                sendfds[1] = sendsock;
                if (send_array_rights(sd, sendfds, 2) == -1)
                    exit(-1);
                break;
            case 4:
                if (send_ttl_rights(sd, sendsock) == -1)
                    exit(-1);
                break;
            default:

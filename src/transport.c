...
int
transportSigSafeSend(transport_t *trans, const char *msg, size_t len) {
    int res = -1;
    if (!trans || !msg || trans->type != CFG_FILE || trans->file.stream == NULL) {
        // TODO: Add support for send variant (test it)
        return res;
    }
    int fd = 0;

    if (trans->file.stdout) {
        fd = STDOUT_FILENO;
    } else if (trans->file.stderr) {
        fd = STDERR_FILENO;
    } else {
        fd = scope_open(trans->file.path, O_WRONLY|O_APPEND, 0666);
    }
    ...
    int bytes = scope_write(fd, msg, len);
    if (bytes != len) {
        if (scope_errno == EBADF) {
            DBG("%d %d", bytes, len);
            transportDisconnect(trans);

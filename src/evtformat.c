...
#define FMTERR(e, j)                                                             \
    {                                                                            \
        DBG("time=%s src=%s data=%p json=%p", e.timestamp, e.src, e.data, json); \
        if (e.data)                                                              \
            cJSON_Delete(e.data);                                                \
        if (j)                                                                   \
            cJSON_Delete(j);                                                     \
...
static enum_map_t watchTypeMap[] = {{"file", CFG_SRC_FILE},     {"console", CFG_SRC_CONSOLE}, {"syslog", CFG_SRC_SYSLOG},
...
evtFormatDestroy(evt_fmt_t **evt)
{
    ...
    watch_t src;
    for (src = CFG_SRC_FILE; src < CFG_SRC_MAX; src++) {
        if (edestroy->value_re[src].valid)
            regfree(&edestroy->value_re[src].re);
        if (edestroy->field_re[src].valid)
            regfree(&edestroy->field_re[src].re);
        if (edestroy->name_re[src].valid)
            regfree(&edestroy->name_re[src].re);
    }
...
{
    if (src < CFG_SRC_MAX) {
        if (evt && evt->field_re[src].valid)
            return &evt->field_re[src].re;
        static local_re_t default_re[CFG_SRC_MAX];
        if (!default_re[src].valid) {
...
{
    if (src < CFG_SRC_MAX) {
        if (evt && evt->name_re[src].valid)
            return &evt->name_re[src].re;
        static local_re_t default_re[CFG_SRC_MAX];
        if (!default_re[src].valid) {
...
anyValueFieldMatches(regex_t *filter, event_t *metric)
{
    ...
    if (valbuf[0]) {
        if (!regexec_wrapper(filter, valbuf, 0, NULL, 0))
            return MATCH_FOUND;
    }
    ...
    for (fld = metric->fields; fld->value_type != FMT_END; fld++) {
        ...
        if (str && !regexec_wrapper(filter, str, 0, NULL, 0))
            return MATCH_FOUND;
    }
...
fmtEventJson(evt_fmt_t *efmt, event_format_t *sev)
{
    ...
    if (!cJSON_AddStringToObjLN(json, SOURCETYPE, valToStr(watchTypeMap, sev->sourcetype)))
        goto err;

    if (!cJSON_AddNumberToObjLN(json, TIME, sev->timestamp))
        goto err;
    if (!cJSON_AddStringToObjLN(json, SOURCE, sev->src))
        goto err;
    if (!cJSON_AddStringToObjLN(json, HOST, sev->proc->hostname))
        goto err;
    if (!cJSON_AddStringToObjLN(json, PROCNAME, sev->proc->procname))
        goto err;
    if (!cJSON_AddStringToObjLN(json, CMDNAME, sev->proc->cmd))
        goto err;
    if (!cJSON_AddNumberToObjLN(json, PID, sev->proc->pid))
        goto err;

    if (efmt)
        addCustomJsonFields(efmt, json);
    cJSON_AddItemToObjectCS(json, DATA, sev->data);

    return json;
err:
    DBG("time=%s src=%s data=%p host=%s json=%p", sev->timestamp, sev->src, sev->data, sev->proc->hostname, json);
    if (json)
        cJSON_Delete(json);
    if (sev->data)
        cJSON_Delete(sev->data);
...
addJsonFields(event_field_t *fields, regex_t *fieldFilter, cJSON *json)
{
    ...
    for (fld = fields; fld->value_type != FMT_END; fld++) {
        ...
        if (fieldFilter && regexec_wrapper(fieldFilter, fld->name, 0, NULL, 0))
            continue;

        // skip if this field is not used in events
        if (fld->event_usage == FALSE)
            continue;

        if (fld->value_type == FMT_STR) {
            if (!cJSON_AddStringToObjLN(json, fld->name, fld->value.str))
                return FALSE;
        } else if (fld->value_type == FMT_NUM) {
            if (!cJSON_AddNumberToObjLN(json, fld->name, fld->value.num))
                return FALSE;
        } else {
            DBG("bad field type");
        }
...
fmtMetricJson(event_t *metric, regex_t *fieldFilter, watch_t src)
{
    ...
    if (src == CFG_SRC_METRIC) {
        if (!cJSON_AddStringToObjLN(json, "_metric", metric->name))
            goto err;
        metric_type = metricTypeStr(metric->type);
        if (!cJSON_AddStringToObjLN(json, "_metric_type", metric_type))
            goto err;
        switch (metric->value.type) {
            case FMT_INT:
                if (!cJSON_AddNumberToObjLN(json, "_value", metric->value.integer))
                    goto err;
                break;
            case FMT_FLT:
                if (!cJSON_AddNumberToObjLN(json, "_value", metric->value.floating))
                    goto err;
                break;
            default:
                DBG(NULL);
    ...
    // Add fields
    if (!addJsonFields(metric->fields, fieldFilter, json))
        goto err;
    return json;

err:
    DBG("_metric=%s _metric_type=%s _value=%lld, fields=%p, json=%p", metric->name, metric_type, metric->value, metric->fields, json);
    if (json)
        cJSON_Delete(json);
...
evtFormatHelper(evt_fmt_t *evt, event_t *metric, uint64_t uid, proc_id_t *proc, watch_t src)
{
    ...
    if (!evtFormatSourceEnabled(evt, src) || !(filter = evtFormatNameFilter(evt, src)) || (regexec_wrapper(filter, metric->name, 0, NULL, 0))) {
        return NULL;
    }
